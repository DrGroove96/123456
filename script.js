...
async function submitTemperatureFeedback() {
    ...
    try {
        // Send temperature report to server
        const response = await fetch('/report-temperature', {
            method: 'POST',
            headers: {
...
function initializeTemperatureUpdates() {
    temperatureEventSource = new EventSource('/temperature-updates');
    
    temperatureEventSource.onmessage = function(event) {
    ...
    temperatureEventSource.onerror = function() {
